...
export default function ProductCard(props) {
  ...
  return (
    <div className="product-card">
      <Link className="testImg" to={"/product/" + props.product.fdcId}>
        <ImageScraping
          productId={props.product.fdcId}
          productCategory={props.product.foodCategory}
        />
      </Link>
      <div className="product-name">
        <button onClick={() => (add ? handleRemove() : handleAdd())}>
          {add ? (
        ...
        <p>{add ? "Liked!" : ""} </p>
        <h1>
          {props.product.lowercaseDescription == undefined
            ? props.product.description
            : props.product.lowercaseDescription}
        </h1>
        {display}
      </div>
    </div>
