...
export default function UserRatings({ user }) {
  ...
  return (
    <>
      <Link to="/userProfile">
        <button className="buttons">Go back to User Profile</button>
      </Link>
      <div className="allRatings">
        {ratings.map((element, idx) => {
          return (
            <span className="user-info-holder">
              <div className="user-card ratings" key={idx}>
                <button
                  className="buttons"
                  onClick={() => handleRemove(element.objectId)}
                >
                  Delete?
                </button>
                ...
                <p className="skill">
                  Comment left on{" "}
                  <Link to={`/product/${element.productId}`}>this product</Link>
                </p>
              </div>
            </span>
          );
