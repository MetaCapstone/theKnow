...
export default function ProductDetail(props) {
  ...
  async function getData() {
    ...
    let response = await axios
      .get(
        `https://api.nal.usda.gov/fdc/v1/food/${params.productId}?&api_key=oDWPyC6zdMmMtm1ZtHe7prk8I18ZaFR5ShQ7QpYB&pageSize=20`,
        {
          headers: {
    ...
    async function responses() {
      ...
      if (response.data == undefined) {
        setProductState({
          ...
          brandOwner: response.company,
          value: response.healthRating,
        });
      }
      getData();
  ...
  async function getRatingExists() {
    try {
      const res = await axios.get(
        `http://localhost:3001/rating_add/${params.productId}`
      );
  ...
  async function getAvgRatings() {
    try {
      const res = await axios.get(
        `http://localhost:3001/ratings/${params.productId}`
      );
    ...
    return (
      <div className="details">
          ...
          <div className="health">
            {productState.value == undefined ? (
              <HealthRating
                ...
                getRatingExists={getRatingExists}
                addRating={addRating}
              />
            ) : (
              <Rating value={productState.value} readOnly />
            )}
          </div>
        </div>
        ...
        <div className="rate">
          ...
          <label htmlFor="star5" title="Really Good!!">
            5 stars
          </label>
          ...
          <label htmlFor="star1" title="Really Bad :(">
            1 star
          </label>
        </div>
        <div className="review">
